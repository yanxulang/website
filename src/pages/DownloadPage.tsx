...
import { binaryDownloadsReady, releaseDownloadUrl, releaseUrl, sourceArchiveUrl, unixInstall, version, vscodeRepository, vscodeVersion, windowsInstall } from "../lib";
...
export function DownloadPage() {
  ...
  return (
    <>
      <header className="border-b border-black/10 py-16 dark:border-white/10"><div className="shell"><p className="text-xs text-muted dark:text-[#aaa69b]"><a href="../" className="no-underline">言序</a> / 下载</p><h1 className="mt-4 font-serif text-[clamp(3rem,6vw,5rem)] leading-[1.02] font-bold tracking-[-.055em]">在你的系统上<br />运行言序。</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-muted dark:text-[#aaa69b]">1.x 发布流程为 Windows、macOS 与 Linux 的主流架构生成可校验压缩包；1.1.3 包含完整父类调用、类型判断、逐跳权限校验的 HTTPS，以及带 DNS 复查、超时和资源上限的 TCP/UDP 套接字。</p><div className="panel mt-8 flex flex-col items-start justify-between gap-5 p-5 sm:flex-row sm:items-center"><div><strong>检测到 {osLabel}</strong><p className="mt-1 text-sm text-muted dark:text-[#aaa69b]">已为你标出推荐构建。当前稳定版本：{version}</p></div><a className="button-secondary" href={releaseUrl}>查看发行状态</a></div></div></header>
