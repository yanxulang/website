...
export function DownloadPage() {
  ...
  return (
    <>
      <header className="border-b border-black/10 py-16 dark:border-white/10"><div className="shell"><p className="text-xs text-muted dark:text-[#aaa69b]"><a href="../" className="no-underline">言序</a> / 下载</p><h1 className="mt-4 font-serif text-[clamp(3rem,6vw,5rem)] leading-[1.02] font-bold tracking-[-.055em]">在你的系统上<br />运行言序。</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-muted dark:text-[#aaa69b]">1.1.6 是工程化稳定性版本：YXB 不能突破宿主权限上限，外部归档受完整资源预算约束，原生扩展和 Release 可追溯性进一步加固。语言核心与言包分别安装、分别发布。</p><div className="panel mt-8 flex flex-col items-start justify-between gap-5 p-5 sm:flex-row sm:items-center"><div><strong>检测到 {osLabel}</strong><p className="mt-1 text-sm text-muted dark:text-[#aaa69b]">已为你标出推荐构建。当前稳定版本：{version}</p></div><a className="button-secondary" href={releaseUrl}>查看发行状态</a></div></div></header>
      ...
      <section id="vscode" className="scroll-mt-24 py-24"><div className="shell"><div className="grid items-end gap-8 lg:grid-cols-[.8fr_1.2fr] lg:gap-16"><div><span className="eyebrow">官方工具</span><h2 className="mt-4 font-serif text-[clamp(2.5rem,5vw,4rem)] leading-[1.08] font-bold tracking-[-.05em]">编辑与<br />工程。</h2></div><p className="text-lg leading-8 text-muted dark:text-[#aaa69b]">VS Code {vscodeVersion} 同步言序 1.1.6 的 25 模块标准库、包索引、YXB 构建、语义编辑与 DAP。言包 {yanbaoVersion} 负责项目、依赖、测试和分发；业务代码完全由言序编写，独立安装包内含专用运行时，因此无需预装言序。</p></div><div className="mt-12 grid gap-4 md:grid-cols-2"><div className="relative overflow-hidden rounded-[1.8rem] bg-night p-8 text-[#fff8ec] sm:p-12"><h3 className="relative z-10 text-3xl font-bold">VS Code 扩展</h3><p className="relative z-10 mt-4 text-white/60">下载 VSIX，在扩展面板中选择“从 VSIX 安装”。</p><a className="relative z-10 mt-7 inline-flex min-h-11 items-center rounded-full bg-[#fff8ec] px-5 text-sm font-semibold text-[#711b14] no-underline" href={`${vscodeRepository}/releases/tag/v${vscodeVersion}`}>下载 {vscodeVersion} →</a></div><div className="rounded-[1.8rem] border border-black/10 p-8 sm:p-12 dark:border-white/10"><h3 className="text-3xl font-bold">纯言序言包</h3><p className="mt-4 text-muted dark:text-[#aaa69b]">一键安装独立 Release；内含专用言序 1.1.6 运行时，无需 Rust 或预装核心。</p><code className="mt-5 block overflow-x-auto whitespace-nowrap rounded-lg bg-black/[.04] p-3 text-xs dark:bg-white/[.05]">{yanbaoUnixInstall}</code><a className="button-secondary mt-7" href={`${yanbaoRepository}/releases`}>下载言包 {yanbaoVersion} →</a></div></div></div></section>
