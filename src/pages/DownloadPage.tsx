...
export function DownloadPage() {
  ...
  return (
    <>
      <header className="border-b border-black/10 py-16 dark:border-white/10"><div className="shell"><p className="text-xs text-muted dark:text-[#aaa69b]"><a href="../" className="no-underline">言序</a> / 下载</p><h1 className="mt-4 font-serif text-[clamp(3rem,6vw,5rem)] leading-[1.02] font-bold tracking-[-.055em]">在你的系统上<br />运行言序。</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-muted dark:text-[#aaa69b]">1.x 发布流程为 Windows、macOS 与 Linux 的主流架构生成可校验压缩包；1.1.4 新增字节串、二进制文件/HTTP/TCP/UDP、安全随机、HMAC、明确 EOF 与服务器辅助能力。</p><div className="panel mt-8 flex flex-col items-start justify-between gap-5 p-5 sm:flex-row sm:items-center"><div><strong>检测到 {osLabel}</strong><p className="mt-1 text-sm text-muted dark:text-[#aaa69b]">已为你标出推荐构建。当前稳定版本：{version}</p></div><a className="button-secondary" href={releaseUrl}>查看发行状态</a></div></div></header>
